...
async function verifyMembership(client, link, telegramResult = null) {
  try {
    ...
    return participant ? { verified: true, state: 'JOINED', evidence: { participantClass: participant.className || null, chatId: String(entity?.channelId || entity?.chatId || entity?.id || ''), username: entity?.username || null, title: entity?.title || null, verifiedAt: new Date().toISOString() } } : { verified: false, state: 'NOT_VERIFIED', reason: 'EMPTY_PARTICIPANT' };
  } catch (error) {
...
const Service = {
    ...
    const locked = await withAdvisoryLock(lockKey, async () => {
      ...
      try {
        ...
        try {
          ...
          await this.finishOperation(operation, { status: 'SUCCESS', resultCode: 'SUCCESS', membershipState: 'JOINED', durationMs: Date.now() - started, verificationEvidence: verification.evidence || {} });
        } catch (error) {
          const classified = classifyError(error);
          ...
          if (classified.resultCode === 'ALREADY_MEMBER') { const existingVerification = await verifyMembership(worker.client, link, null); if (existingVerification.verified) { await this.finishOperation(operation, { status: 'SUCCESS', resultCode: 'ALREADY_MEMBER', membershipState: 'ALREADY_MEMBER', verificationEvidence: existingVerification.evidence || {}, durationMs: Date.now() - started }); return { status: 'SUCCESS', resultCode: 'ALREADY_MEMBER' }; } await this.finishOperation(operation, { status: 'FAILED', resultCode: 'NOT_VERIFIED', errorCode: 'NOT_VERIFIED', errorMessage: 'Telegram قال إن الحساب عضو، لكن فحص العضوية الفعلي لم يؤكد ذلك', membershipState: existingVerification.state || 'NOT_VERIFIED', durationMs: Date.now() - started }); return { status: 'FAILED', resultCode: 'NOT_VERIFIED' }; }
          if (canRetry) {
  ...
  async finishOperation(operation, result) {
    ...
    await withTransaction(async client => {
      ...
      const linkRow = (await client.query(`SELECT normalized_url,telegram_identifier,link_type FROM telegram_automation_links WHERE id=$1`, [operation.link_id])).rows[0];
        const globalHash = linkRow ? GlobalJoinRegistry.normalize(linkRow.normalized_url)?.urlHash || '' : '';
        const globalRow = linkRow ? (await client.query(`SELECT id FROM telegram_global_join_links WHERE normalized_url=$1 OR url_hash=$2 ORDER BY CASE WHEN normalized_url=$1 THEN 0 ELSE 1 END LIMIT 1 FOR UPDATE`, [linkRow.normalized_url, globalHash])).rows[0] : null;
      if (successful) {
        ...
        if (globalRow) await GlobalJoinRegistry.markJoined(client, { registryId: globalRow.id, accountId: operation.account_id, operationId: operation.id, userId: operation.user_id, normalizedUrl: linkRow.normalized_url, chatId: result.telegramChatId || result.chatId || result.verificationEvidence?.chatId || null, username: result.telegramUsername || result.verificationEvidence?.username || null, status: result.resultCode === 'ALREADY_MEMBER' ? 'ALREADY_MEMBER' : 'JOINED' });
        if (result.verificationEvidence?.chatId) await client.query(`INSERT INTO telegram_account_group_memberships(user_id,former_account_id,group_id,group_title,group_username,source_link,status,discovered_at) VALUES($1,$2,$3,$4,$5,$6,'ACTIVE',NOW()) ON CONFLICT(user_id,former_account_id,group_id) DO UPDATE SET group_title=COALESCE(EXCLUDED.group_title,telegram_account_group_memberships.group_title),group_username=COALESCE(EXCLUDED.group_username,telegram_account_group_memberships.group_username),source_link=COALESCE(EXCLUDED.source_link,telegram_account_group_memberships.source_link),status='ACTIVE',last_error=NULL,updated_at=NOW()`, [operation.user_id, operation.account_id, String(result.verificationEvidence.chatId), result.verificationEvidence.title || null, result.verificationEvidence.username || null, linkRow.normalized_url]);
      } else {
