...
const Service = {
  ...
  async processGeminiJob(job) {
    ...
    try {
      ...
      await query(`UPDATE telegram_smart_results SET ai_status=$1,status=CASE WHEN $1='failed' THEN 'analysis_failed' ELSE status END,ai_error=$2,retry_count=$3,updated_at=NOW() WHERE id=$4 AND user_id=$5`, [failureStatus, errorMessage, retryCount, resultId, userId]).catch(() => {});
